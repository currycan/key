...
重构 Bash 脚本（如 `entrypoint.sh`）时，**必须**严格遵循以下流程：
...
4. **跨平台兼容**: 所有 `sed -i` 改用封装函数 `_sed_i()`，管道末端 `tr | head -c` 须加 `|| true` 防 SIGPIPE 导致 `pipefail` 误退出，避免使用 Linux 专属命令（如 `shuf`，改用 `$(( RANDOM % N + base ))`）
5. **函数声明顺序**:
   - 基础工具函数（无业务状态依赖）必须先于业务函数声明，分区如下：
   ...
   - 业务函数之间按**运行依赖链**排序（被依赖的先声明）
   - 参照 `templates/supervisord/daemon.ini` 的 priority 值理解各组件启动顺序，`main_init` 内的步骤注释须标注对应的 daemon.ini priority 值
6. **结构清晰**: 大脚本用 `§N 段落名` 注释分段（建议 14-16 段），函数以注释标注用途、参数和依赖关系
