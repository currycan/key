...
"
    # grpc-reality
    local CLASH_GRPC_REALITY="- {name: \"${GEOIP_INFO}|${NODE_NAME}|grpc-reality\", type: vless, server: ${DOMAIN}, port: ${PORT_GRPC_REALITY}, uuid: ${SB_UUID}, network: grpc, tls: true, udp: true, flow: , client-fingerprint: chrome, servername: addons.mozilla.org, grpc-opts: {  grpc-service-name: \"grpc\" }, reality-opts: { public-key: ${SB_REALITY_PUBLIC_KEY}, short-id: \"\" }, smux: { enabled: true, protocol: 'h2mux', padding: true, max-connections: '8', min-streams: '16', statistic: true, only-tcp: false }, brutal-opts: { enabled: ${IS_BRUTAL}, up: '2500 Mbps', down: '2500 Mbps' } }"
    CLASH_SUBSCRIBE+="  $CLASH_GRPC_REALITY
"
    # anytls
    local CLASH_ANYTLS="- {name: \"${GEOIP_INFO}|${NODE_NAME}|anytls\", type: anytls, server: ${NODE_IP}, port: ${PORT_ANYTLS}, password: ${SB_UUID}, client-fingerprint: chrome, udp: true, idle-session-check-interval: 30, idle-session-timeout: 30, skip-cert-verify: true }"
    CLASH_SUBSCRIBE+="  $CLASH_ANYTLS
"
    # vmess ws tls
    local CLASH_VMESS_WS_TLS="- {\"name\":\"${GEOIP_INFO}|${NODE_NAME}|${DOMAIN}-VMESS\",\"type\":\"vmess\",\"server\":\"${DOMAIN}\",\"port\":${LISTENING_PORT},\"cipher\":\"auto\",\"uuid\":\"${V2RAY_UUID}\",\"alterId\":${ALTERID},\"tls\":true,\"${NETWORK}\":\"ws\",\"ws-opts\":{\"path\":\"/${V2RAY_URL_PATH}\",\"headers\":{\"Host\":\"${DOMAIN}\"}},\"servername\":\"${DOMAIN}\"}"
    CLASH_SUBSCRIBE+="  $CLASH_VMESS_WS_TLS
"
    # vless XTLS(Vision)+Reality直连
    local CLASH_VLESS_XTLS_REALITY="- {\"type\":\"vless\",\"name\":\"${GEOIP_INFO}|${NODE_NAME}|${NODE_NAME}|XTLS(Vision)+Reality直连\",\"server\":\"${DOMAIN}\",\"port\":${LISTENING_PORT},\"uuid\":\"${XRAY_REALITY_SHORTID}\",\"tls\":true,\"flow\":\"xtls-rprx-vision\",\"client-fingerprint\":\"chrome\",\"skip-cert-verify\":false,\"reality-opts\":{\"public-key\":\"${XRAY_REALITY_PUBLIC_KEY}\",\"short-id\":\"${XRAY_REALITY_SHORTID}\",\"_spider-x\":\"/\"},\"network\":\"tcp\",\"servername\":\"${DOMAIN}\"}"
    CLASH_SUBSCRIBE+="  $CLASH_VLESS_XTLS_REALITY
"
    # vless Xhttp+Reality直连
    local CLASH_VLESS_XHTTP_REALITY="- {\"type\":\"vless\",\"name\":\"${GEOIP_INFO}|${NODE_NAME}|${NODE_NAME}|Xhttp+Reality直连\",\"server\":\"${DOMAIN}\",\"port\":${LISTENING_PORT},\"uuid\":\"${XRAY_XHTTP_UUID}\",\"tls\":true,\"client-fingerprint\":\"chrome\",\"skip-cert-verify\":false,\"reality-opts\":{\"public-key\":\"${XRAY_REALITY_PUBLIC_KEY}\",\"short-id\":\"${XRAY_REALITY_SHORTID}\"},\"network\":\"xhttp\",\"xhttp-opts\":{\"path\":\"/${XRAY_XHTTP_URL_PATH}\"},\"servername\":\"${DOMAIN}\"}"
    CLASH_SUBSCRIBE+="  $CLASH_VLESS_XHTTP_REALITY
"
    # vless 上行Xhttp+TLS+CDN | 下行Xhttp+Reality
    local CLASH_VLESS_XHTTP_TLS_CDN_UP_REALITY_DOWN="- {\"type\":\"vless\",\"name\":\"${GEOIP_INFO}|${NODE_NAME}|${NODE_NAME}|上行Xhttp+TLS+CDN|下行Xhttp+Reality\",\"server\":\"${DOMAIN}\",\"port\":${LISTENING_PORT},\"uuid\":\"${XRAY_XHTTP_UUID}\",\"tls\":true,\"client-fingerprint\":\"chrome\",\"alpn\":[\"h2\"],\"skip-cert-verify\":false,\"network\":\"xhttp\",\"xhttp-opts\":{\"headers\":{\"Host\":\"${CDNDOMAIN}\"},\"path\":\"/${XRAY_XHTTP_URL_PATH}\"},\"servername\":\"${CDNDOMAIN}\"}"
    CLASH_SUBSCRIBE+="  $CLASH_VLESS_XHTTP_TLS_CDN_UP_REALITY_DOWN
"
    # vless 上行Xhttp+Reality | 下行Xhttp+TLS+CDN
    local CLASH_VLESS_XHTTP_REALITY_UP_TLS_CDN_DOWN="- {\"type\":\"vless\",\"name\":\"${GEOIP_INFO}|${NODE_NAME}|${NODE_NAME}|上行Xhttp+Reality|下行 Xhttp+TLS+CDN\",\"server\":\"${DOMAIN}\",\"port\":${LISTENING_PORT},\"uuid\":\"${XRAY_XHTTP_UUID}\",\"tls\":true,\"client-fingerprint\":\"chrome\",\"alpn\":[\"h2\"],\"skip-cert-verify\":false,\"reality-opts\":{\"public-key\":\"${XRAY_REALITY_PUBLIC_KEY}\",\"short-id\":\"${XRAY_REALITY_SHORTID}\"},\"network\":\"xhttp\",\"xhttp-opts\":{\"path\":\"/${XRAY_XHTTP_URL_PATH}\"},\"servername\":\"${DOMAIN}\"}"
    CLASH_SUBSCRIBE+="  $CLASH_VLESS_XHTTP_REALITY_UP_TLS_CDN_DOWN
"
    # vless Xhttp+TLS+CDN上下行不分离
    local CLASH_VLESS_XHTTP_TLS_CDN="- {\"type\":\"vless\",\"name\":\"${GEOIP_INFO}|${NODE_NAME}|${NODE_NAME}|Xhttp+TLS+CDN上下行不分离\",\"server\":\"${DOMAIN}\",\"port\":${LISTENING_PORT},\"uuid\":\"${XRAY_XHTTP_UUID}\",\"tls\":true,\"client-fingerprint\":\"chrome\",\"alpn\":[\"h2\"],\"skip-cert-verify\":false,\"network\":\"xhttp\",\"xhttp-opts\":{\"headers\":{\"Host\":\"${CDNDOMAIN}\"},\"path\":\"/${XRAY_XHTTP_URL_PATH}\"},\"servername\":\"${CDNDOMAIN}\"}"
...
"
    ...
    V2RAYN_SUBSCRIBE+="
vless://${XRAY_REALITY_UUID}@${DOMAIN}:${LISTENING_PORT}?encryption=none&flow=xtls-rprx-vision&security=reality&sni=${DOMAIN}&fp=chrome&pbk=${XRAY_REALITY_PUBLIC_KEY}&sid=${XRAY_REALITY_SHORTID}&spx=%2F&type=tcp&headerType=none#${GEOIP_INFO}|${name}|XTLS(Vision)+Reality直连
"
    # Xhttp+Reality直连
    V2RAYN_SUBSCRIBE+="
...
"
    ...
    V2RAYN_SUBSCRIBE+="
vless://${XRAY_XHTTP_UUID}@${DOMAIN}:${LISTENING_PORT}?encryption=none&security=tls&sni=${CDNDOMAIN}&alpn=h2&fp=chrome&pbk=${XRAY_REALITY_PUBLIC_KEY}&sid=${XRAY_REALITY_SHORTID}&type=xhttp&host=${CDNDOMAIN}&path=%2F${XRAY_XHTTP_URL_PATH}&mode=auto#${GEOIP_INFO}|${name}|Xhttp+TLS+CDN上下行不分离
"
