...
"
    # ShadowTLS
#     V2RAYN_SUBSCRIBE+="
# ss://$(echo -n "${SS_METHOD}:${RANDOM_PASSWORD}@${DOMAIN}:${PORT_SHADOWTLS}/?plugin=shadow-tls;host=${SINGBOX_SERVER_NAME};password=${SB_UUID};version=3" | base64 -w0)#${REGION_INFO}|${NODE_NAME}|ShadowTLS
# "
    # shadowsocks
    V2RAYN_SUBSCRIBE+="
ss://$(echo -n "${SS_METHOD}:${RANDOM_PASSWORD}@${DOMAIN}:${PORT_SHADOWSOCKS}" | base64 -w0)#${REGION_INFO}|${NODE_NAME}|shadowsocks
"
    # anytls
    V2RAYN_SUBSCRIBE+="
# 需把 tls 里的 inSecure 设置为 true
