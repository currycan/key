...
generate_links() {
    ...
    [[ -n "${HY2_HOP_RANGE:-}" ]] && hy2_mport_param="&mport=${HY2_LISTEN_PORT},${HY2_HOP_RANGE%-*}-${HY2_HOP_RANGE#*-}"
    local link_hysteria2="hysteria2://${SB_UUID}@${DOMAIN}:${HY2_LISTEN_PORT}/?sni=${DOMAIN}&${h2_alpn}${hy2_mport_param}#${FLAG_PREFIX}Hysteria2 ✈ ${region_name}${NODE_SUFFIX}"
    local tuic_mport_param=""
    [[ -n "${TUIC_HOP_RANGE:-}" ]] && tuic_mport_param="&mport=${TUIC_LISTEN_PORT},${TUIC_HOP_RANGE%-*}-${TUIC_HOP_RANGE#*-}"
    local link_tuic="tuic://${SB_UUID}:${SB_UUID}@${DOMAIN}:${TUIC_LISTEN_PORT}?${h2_alpn}&congestion_control=bbr${tuic_mport_param}#${FLAG_PREFIX}TUIC ✈ ${region_name}${NODE_SUFFIX}"
    local link_anytls="anytls://${SB_UUID}@${ANYTLS_DOMAIN}:${LISTENING_PORT}?security=tls&sni=${ANYTLS_DOMAIN}&type=tcp#${FLAG_PREFIX}AnyTLS ✈ ${region_name}${NODE_SUFFIX}"
