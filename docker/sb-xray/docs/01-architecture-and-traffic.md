...
> 本文档深入剖析 SB-Xray 的核心架构设计——从 Nginx 边界网关的流量拦截与分发，到双引擎内核的协议处理，再到容器启动的五大生命周期扇区——进行全景式解读。
...
容器在每次启动（或执行 `docker compose restart`）时，并不会盲目拉起核心，而是由 `scripts/entrypoint.sh` 执行一套严密的**五大生命周期扇区 (Five-Sector Lifecycle)**。
...
* **定位**：纯粹被动调用的函数合集。
* **代表组件**：
  * `ensure_var`：采用先计算后 `unset → append` 形式完成变量内存读出与物理文件的安全锁定。
  * `http_probe`：**所有流媒体和 AI 探测网关的通用核基建**，封装了带伪装 (UA)、短超时 (3s) 和强制断后重试的 `curl` 探测器。
