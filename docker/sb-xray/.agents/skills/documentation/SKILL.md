...
### 代码修改完成后主动向用户确认是否同步文档

每次代码修改完成后，**必须主动询问用户**：本次修改是否已足够完整、稳定，可以同步更新文档？

- 若用户确认 → 立即执行 `/doc-sync`，按映射表更新对应文档章节
- 若用户说"还没完" → 记下待同步内容，等用户确认后再统一更新
- 禁止等用户主动问"文档更新了吗"才去检查

> 目的：确保文档始终与代码行为保持一致，避免文档滞后。
